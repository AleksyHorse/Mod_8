...
def homepage():
    ...
    if list_type not in types:
        list_type="popular"
    movies = tbdb_client.get_movies(how_many=8, kind=list_type)
...
def movie_details(movie_id):
    det=tbdb_client.get_single_movie(movie_id)
    cast=tbdb_client.get_single_movie_cast(movie_id)[:8]
    picture = tbdb_client.random_get_backdrop(movie_id)
    return render_template("movie_details.html",cast=cast, movie=det, movie_id=movie_id, picture=picture)

@app.context_processor
def utility_processor():
    def tmdb_image_url(path, size):
        return tbdb_client.get_poster_url(path, size)
